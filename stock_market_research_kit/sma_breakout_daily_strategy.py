...
def run_sma_breakout_daily_strategy(daily_candles):
    # Array to store trades
    trades = []

    # Current combos being formed
    current_combos = []  # матрица с массивом свечей, которые прошла каждая сделка
    active_trades = []  # Trade[]
...
def look_for_entry(
    ...
    if adr20 < min_adr_percent: # отбрасываем свечи с низким adr (зачем-то)
        return None
